...
function review_admin_redirect_url(): string {
    ...
    return 'review-products.php?' . http_build_query($params);
}
...
function recalc_product_rating(mysqli $conn, int $product_id) {
    $conn->query("UPDATE products p SET p.average_rating = (SELECT IFNULL(AVG(rating), 0) FROM product_reviews WHERE product_id = $product_id AND status = 'Approved'), p.review_count = (SELECT COUNT(*) FROM product_reviews WHERE product_id = $product_id AND status = 'Approved') WHERE p.id = $product_id");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['review_approve'])) {
    ...
    if ($id <= 0) {
        ...
        if (!$row || $row['status'] !== 'Pending Approval') {
            ...
            $upd = $conn->prepare("UPDATE product_reviews SET status = 'Approved', previous_rating = NULL, previous_comment = NULL WHERE id = ? AND status = 'Pending Approval'");
            $upd->bind_param('i', $id);
            if ($upd->execute() && $upd->affected_rows > 0) {
                recalc_product_rating($conn, $row['product_id']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['review_reject_final'])) {
    ...
    if ($id <= 0 || $reason === '') {
        ...
        if (!$row || $row['status'] !== 'Pending Approval') {
            ...
            $check_prev = $conn->prepare("SELECT previous_rating, previous_comment FROM product_reviews WHERE id = ?");
            ...
            if ($prev_row && $prev_row['previous_rating'] !== null) {
                ...
                $upd = $conn->prepare("UPDATE product_reviews SET status = 'Approved', rating = previous_rating, comment = previous_comment, previous_rating = NULL, previous_comment = NULL WHERE id = ? AND status = 'Pending Approval'");
            } else {
                $upd = $conn->prepare("UPDATE product_reviews SET status = 'Rejected' WHERE id = ? AND status = 'Pending Approval'");
            }
            $upd->bind_param('i', $id);
            if ($upd->execute() && $upd->affected_rows > 0) {
                $msg = 'Your review for "' . $row['product_title'] . '" was rejected. Reason: ' . $reason;
                review_notify_user($conn, (int)$row['user_id'], $msg);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['review_delete'])) {
    ...
    if ($id <= 0 || $reason === '') {
        ...
        if ($row) {
            $del = $conn->prepare('DELETE FROM product_reviews WHERE id = ?');
            $del->bind_param('i', $id);
            if ($del->execute() && $del->affected_rows > 0) {
                ...
                $msg = 'Your review for "' . $row['product_title'] . '" was removed by an administrator. Reason: ' . $reason;
                review_notify_user($conn, (int)$row['user_id'], $msg);
...
<head>
    ...
    <title>SkateShop | MODERATE REVIEWS</title>
...
<section class="admin-layout container">
    ...
    <main class="admin-main">
        <div class="top-action-bar">
            <div>
                <h2 class="glitch-text-admin">REVIEW <span class="text-primary">PRODUCTS</span></h2>
                <p class="admin-text-shop">MODERATE GEAR REVIEWS</p>
            </div>
        </div>
            ...
            <form method="get" action="review-products.php" class="search-filter-form" id="review-filter-form">
                ...
                <div class="filter-actions" style="margin-top:22px;">
                    ...
                    <?php if ($search_q !== '' || $status_filter !== 'all' || $sort_filter !== 'newest'): ?>
                        <a href="review-products.php" class="btn-reset">RESET</a>
                    <?php endif; ?>
                </div>
            </form>
        </div>
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        <?php foreach ($rows as $item):
                            ...
                            $title_short = mb_strlen($item['product_title']) > 32 ? mb_substr($item['product_title'], 0, 29) . '…' : $item['product_title'];
                            $status = $item['status'];
                            ?>
                            ...
                            <div id="<?php echo htmlspecialchars($mid); ?>" class="modal-overlay">
                                <div class="modal-content modal-content-lg">
                                    ...
                                    <div style="margin-bottom: 20px; overflow: hidden;">
                                        <img src="<?php echo htmlspecialchars($item['product_image']); ?>" class="product-thumb-modal" alt="Product Image">
                                        ...
                                        <div class="detail-row" style="margin-top:0.5rem;"><span class="detail-label">PRODUCT:</span> <span><?php echo htmlspecialchars($item['product_title']); ?></span></div>
                                    ...
                                    <div class="review-modal-body-preview"><?php if (!empty($item['comment'])): ?><?php echo htmlspecialchars($item['comment']); ?><?php else: ?><em style="color: #888;">No comment provided.</em><?php endif; ?></div>
                                     ...
                                     <?php if ($status === 'Pending Approval'): ?>
                                        <form method="post" action="review-products.php" class="admin-form" style="margin-top:1.5rem;">
                            ...
                            <?php if ($status === 'Pending Approval'): ?>
                                <div id="<?php echo htmlspecialchars($reject_mid); ?>" class="modal-overlay modal-top-layer">
                                    <div class="modal-content">
                                        ...
                                        <form method="post" action="review-products.php" class="admin-form">
                                            <?php echo $review_redirect_hidden; ?>
                                            <input type="hidden" name="review_id" value="<?php echo (int)$item['id']; ?>">
