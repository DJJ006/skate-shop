...
function qna_admin_redirect_url(): string {
    global $qna_redirect_allowed;
    $st = $_POST['redirect_status'] ?? 'all';
    if (!in_array($st, $qna_redirect_allowed, true)) {
        $st = 'all';
    }
    ...
    return 'review-qna.php?' . http_build_query($params);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['qna_publish'])) {
    $id = (int)$_POST['qna_id'];
    $admin_answer = trim($_POST['admin_answer'] ?? '');

    if ($id <= 0 || $admin_answer === '') {
        $_SESSION['msg'] = 'ADMIN ANSWER IS REQUIRED TO PUBLISH.';
        $_SESSION['msg_type'] = 'error';
    } else {
        $info = $conn->prepare('SELECT user_id, title, status FROM community_qna WHERE id = ?');
        ...
        if (!$row || $row['status'] !== 'pending') {
            ...
            $upd = $conn->prepare("UPDATE community_qna SET status = 'published', admin_answer = ?, published_at = NOW() WHERE id = ? AND status = 'pending'");
            $upd->bind_param('si', $admin_answer, $id);
            if ($upd->execute() && $upd->affected_rows > 0) {
                $title = $row['title'];
                $uid = (int)$row['user_id'];
                $msg = 'Your Q&A "' . $title . '" has been published with an official answer.';
                qna_notify_user($conn, $uid, $msg);
                $_SESSION['msg'] = 'Q&A PUBLISHED & USER NOTIFIED.';
    ...
    header('Location: ' . qna_admin_redirect_url());
    exit();
}

// --- POST: reject ---
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['qna_reject_final'])) {
    $id = (int)$_POST['qna_id'];
    ...
    if ($id <= 0 || $reason === '') {
        ...
        if (!$row || $row['status'] !== 'pending') {
            ...
            $del = $conn->prepare('DELETE FROM community_qna WHERE id = ? AND status = ?');
            $st = 'pending';
            $del->bind_param('is', $id, $st);
            if ($del->execute() && $del->affected_rows > 0) {
                $msg = 'Your Q&A "' . $row['title'] . '" was rejected by an administrator. Reason: ' . $reason;
                qna_notify_user($conn, (int)$row['user_id'], $msg);
                $_SESSION['msg'] = 'Q&A REJECTED & DELETED. CLIENT NOTIFIED.';
                $_SESSION['msg_type'] = 'success';
            } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['qna_save_answer'])) {
    $id = (int)$_POST['qna_id'];
    $admin_answer = trim($_POST['admin_answer'] ?? '');
    ...
    if ($id <= 0 || $admin_answer === '') {
        ...
        if (!$row || $row['status'] !== 'published') {
            ...
            $upd = $conn->prepare('UPDATE community_qna SET admin_answer = ? WHERE id = ? AND status = ?');
            $st = 'published';
            $upd->bind_param('sis', $admin_answer, $id, $st);
            if ($upd->execute() && $upd->affected_rows > 0) {
                $msg = 'The official answer to your Q&A "' . $row['title'] . '" was updated by an administrator.';
                qna_notify_user($conn, (int)$row['user_id'], $msg);
...
// --- POST: delete ---
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['qna_delete'])) {
    $id = (int)$_POST['qna_id'];
    ...
    if ($id <= 0 || $reason === '') {
        ...
        $info = $conn->prepare('SELECT user_id, title FROM community_qna WHERE id = ?');
        $info->bind_param('i', $id);
        $info->execute();
        $row = $info->get_result()->fetch_assoc();

        if ($row) {
            $del = $conn->prepare('DELETE FROM community_qna WHERE id = ?');
            $del->bind_param('i', $id);
            if ($del->execute() && $del->affected_rows > 0) {
                // Send notification
                $msg = 'Your Q&A "' . $row['title'] . '" was removed by an administrator. Reason: ' . $reason;
                qna_notify_user($conn, (int)$row['user_id'], $msg);
                
                $_SESSION['msg'] = 'Q&A DELETED & USER NOTIFIED.';
                $_SESSION['msg_type'] = 'success';
            } else {
...
// --- List query ---
$status_filter = isset($_GET['status']) && in_array($_GET['status'], $qna_redirect_allowed, true) ? $_GET['status'] : 'all';
...
// COUNT TOTAL RECORDS
$count_res = null;
$count_sql = "SELECT COUNT(*) as total FROM community_qna q WHERE $where_sql";
...
$list_sql = "SELECT q.* FROM community_qna q WHERE $where_sql $order_sql LIMIT ? OFFSET ?";
...
$qna_redirect_hidden = '<input type="hidden" name="redirect_status" value="' . htmlspecialchars($status_filter, ENT_QUOTES, 'UTF-8') . '">'
...
function qna_status_badge_class(string $status): string {
    if ($status === 'pending') {
...
<head>
    ...
    <title>SkateShop | REVIEW Q&A</title>
...
<section class="admin-layout container">
    ...
    <main class="admin-main">
        <div class="top-action-bar">
            <div>
                <h2 class="glitch-text-admin">REVIEW <span class="text-primary">Q&A</span></h2>
        ...
        <?php if ($table_missing): ?>
            <div class="grainy-card card-padding" style="padding: 20px;">
                <p class="admin-text-shop" style="font-weight:700;">The <code>community_qna</code> table is missing. Run the SQL in <code>sql/community_qna.sql</code> on your database, then reload this page.</p>
            </div>
        <?php else: ?>
        ...
        <div class="grainy-card card-padding" style="padding: 20px;">
            <h3 class="admin-table-h3">Q&A <span class="header-span">SUBMISSIONS</span></h3>

            <div class="grainy-card filter-bar" style="margin-bottom: 25px;">
            <form method="get" action="review-qna.php" class="search-filter-form" id="qna-filter-form">
                ...
                <div class="filter-actions" style="margin-top:22px;">
                    ...
                    <?php if ($search_q !== '' || $status_filter !== 'all' || $sort_filter !== 'newest'): ?>
                        <a href="review-qna.php" class="btn-reset">RESET</a>
                    <?php endif; ?>
                </div>
            </form>
        </div>
            ...
            <table class="recent-activity-table">
                ...
                <tbody>
                    <?php if (count($rows) === 0): ?>
                        <tr><td colspan="6" style="text-align:center;font-weight:700;font-style:italic;">NO Q&A ROWS MATCH THIS FILTER.</td></tr>
                    <?php else: ?>
                        <?php foreach ($rows as $item):
                            ...
                            ?>
                             <tr class="<?php echo ($status === 'pending') ? 'pending-row' : ''; ?>">
                                ...
                                <td><span class="<?php echo qna_status_badge_class($status); ?>"><?php echo strtoupper(htmlspecialchars($status)); ?></span></td>
                            ...
                            <div id="<?php echo htmlspecialchars($mid); ?>" class="modal-overlay">
                                <div class="modal-content modal-content-lg">
                                    ...
                                    <h3 class="admin-table-h3 top-title">Q&A <span class="header-span">DETAIL</span></h3>
                                    <p style="font-family:'Staatliches',sans-serif;font-size:1.1rem;color:#666;margin-bottom:1rem;">
                                        ...
                                        <span class="<?php echo qna_status_badge_class($status); ?>"><?php echo strtoupper(htmlspecialchars($status)); ?></span>
                                    </p>
                                    ...
                                    <?php endif; ?>
                                     <?php if ($status === 'pending'): ?>
                                        <form method="post" action="review-qna.php" class="admin-form" style="margin-top:1.5rem;">
                                            <?php echo $qna_redirect_hidden; ?>
                                            <input type="hidden" name="qna_id" value="<?php echo (int)$item['id']; ?>">
                                            
                                            <label>OFFICIAL ADMIN ANSWER (REQUIRED TO PUBLISH)</label>
                                            <textarea name="admin_answer" rows="5" required placeholder="Write the shop’s official response..."></textarea>
                                            
                                            <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 10px; margin-top: 1rem;">
                                                <button type="submit" name="qna_publish" class="btn btn-primary" style="font-size: 1.2rem;">ACCEPT &amp; PUBLISH</button>
                                    ...
                                    <?php elseif ($status === 'published'): ?>
                                        <form method="post" action="review-qna.php" class="admin-form" style="margin-top:1.5rem;">
                                            ...
                                            <textarea name="admin_answer" rows="5" required><?php echo htmlspecialchars($item['admin_answer'] ?? ''); ?></textarea>
                                            
                                            <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 10px; margin-top: 1rem;">
                                                <button type="submit" name="qna_save_answer" class="btn btn-primary" style="font-size: 1.2rem;">SAVE CHANGES</button>
                            ...
                            <?php if ($status === 'pending'): ?>
                                <div id="<?php echo htmlspecialchars($reject_mid); ?>" class="modal-overlay modal-top-layer">
                                    <div class="modal-content">
                                        ...
                                        <form method="post" action="review-qna.php" class="admin-form">
                                            <?php echo $qna_redirect_hidden; ?>
                                            <input type="hidden" name="qna_id" value="<?php echo (int)$item['id']; ?>">
                                            
                                            <label>WHY ARE YOU REJECTING THIS Q&A?</label>
                                            <textarea name="rejection_reason" rows="4" placeholder="E.g. Off-topic, spam, unclear..." required></textarea>
                                            
                                            <button type="submit" name="qna_reject_final" class="btn btn-danger" style="width:100%; margin-top: 1rem; font-size: 1.2rem;">SEND REJECTION</button>
                                        </form>
                                    </div>
                                </div>
                            <?php endif; ?>
                            ...
                            <div id="<?php echo htmlspecialchars($delete_mid); ?>" class="modal-overlay modal-top-layer">
                                <div class="modal-content">
                                    ...
                                    <form method="post" action="review-qna.php" class="admin-form">
                                        ...
                                        <label>WHY ARE YOU DELETING THIS Q&A?</label>
                                        <textarea name="deletion_reason" rows="4" placeholder="E.g. Inappropriate content, duplicate, spam..." required></textarea>
                                        
                                        <p style="margin-top: 1rem; color: var(--primary); font-family: 'Staatliches', sans-serif; font-size: 0.9rem;">* THIS WILL PERMANENTLY REMOVE THE Q&A FROM THE DATABASE.</p>
                                        
                                        <button type="submit" name="qna_delete" class="btn btn-danger" style="width:100%; margin-top: 1rem; font-size: 1.2rem;">PERMANENTLY DELETE</button>
                                    </form>
                                </div>
                            </div>
                            <?php
