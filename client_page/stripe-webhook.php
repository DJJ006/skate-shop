...
if ($event->type === 'checkout.session.completed') {
    ...
    $order_ids_string = isset($session->metadata->order_ids) ? $session->metadata->order_ids : '';
    webhook_log("Order IDs from metadata: " . $order_ids_string);

    if (empty($order_ids_string)) {
        webhook_log("Missing or invalid order_ids");
        http_response_code(400);
        exit('Missing order_ids');
    }
    ...
    try {
        ...
        foreach ($order_ids_array as $order_id_str) {
            ...
            webhook_log("Order " . $order_id . " found with status: " . $order['status']);

            if ($order['status'] === 'PAID') {
                webhook_log("Order " . $order_id . " already processed, skipping.");
                continue;
            }
            ...
            if (!$product) {
                throw new Exception('Product not found: ' . $prod_id);
            }
